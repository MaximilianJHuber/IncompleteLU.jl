...
@propagate_inbounds getindex(v::InsertableSparseVector{Tv}, idx::I) where {Tv,I} = v.values[idx]
@propagate_inbounds setindex!(v::InsertableSparseVector{Tv}, value::Tv, idx::I) where {Tv,I} = v.values[idx] = value
...
are added.
"""
function add!(v::InsertableSparseVector{Tv}, a::Tv, idx::I, prev_idx::Integer) where {Tv,I}
...
"""
@propagate_inbounds add!(v::InsertableSparseVector{Tv}, a::Tv, idx::I) where {Tv,I} = add!(v, a, idx, v.indices.N)
...
"""
function append_col!(A::SparseMatrixCSC{Tv, I}, y::InsertableSparseVector{Tv}, j::Integer, drop::Tv, scale::Tv = one(Tv)) where {Tv}
